...
void main() {
  ...
  test(
    ...
    () async {
      // arrange
      when(repository.createRefuelingForVehicleID(vehicleID, objects.first))
          .thenAnswer((_) async => Right(objects));
      // act
      final result = await usecase(Params(vehicleID: vehicleID, refueling: objects.first));
      // assert
      expect(result, Right(objects));
      verify(repository.createRefuelingForVehicleID(vehicleID, objects.first));
